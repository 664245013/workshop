...
with col1:
    ...
    person_education = st.selectbox("ระดับการศึกษา", model_info['categories'].get('person_education', ['High School', 'Bachelor', 'Master']))
    ...
    person_home_ownership = st.selectbox("สถานะที่อยู่อาศัย", model_info['categories'].get('person_home_ownership', ['RENT', 'OWN', 'MORTGAGE']))

with col2:
    st.subheader("💵 ข้อมูลการกู้")
    loan_amnt = st.number_input("จำนวนเงินกู้ (บาท)", min_value=0, value=10000, step=1000)
    loan_intent = st.selectbox("วัตถุประสงค์", model_info['categories'].get('loan_intent', ['PERSONAL', 'EDUCATION', 'MEDICAL', 'VENTURE']))
